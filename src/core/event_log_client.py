    ...
    def insert(self, data: List[Model], batch_size: int = 100) -> None:
        ...
        try:
            for i in range(0, len(data), batch_size):
                ...
                self._client.insert(
                    data=self._convert_data(batch), column_names=EVENT_LOG_COLUMNS, database=self._schema, table=self._table, )
                logger.info("Batch inserted into ClickHouse", batch_size=len(batch), total_records=len(data), batch_number=(i // batch_size) + 1, )
